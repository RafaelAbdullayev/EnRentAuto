...
│   │   ├── brand.ts           # Логотип и фон: поиск, сохранение, удаление файла
...
│   │       ├── BrandImageUploader.tsx # Загрузка логотипа и фона первого экрана
│   │       └── BookingActions.tsx    # Выдать / принять / отменить
│   └── app/
│       ├── globals.css
│       ├── uploads/[...path]/route.ts # Отдача загруженных фото
│       ├── brand/[kind]/route.ts     # Отдача логотипа и фона (ETag, без пересборки)
...
Обе картинки загружает администратор: **Оформление и тексты** —
перетащите файл в нужное поле. Изменения видны сразу, пересобирать проект
и перезапускать сервис не нужно.

| Что | Требования | Где видно |
|---|---|---|
| Логотип | PNG с прозрачным фоном, высота от 200 px | Шапка, подвал, страница входа, админка |
| Фон первого экрана | JPG или WEBP, от 1920×1080 | Главная страница, первый экран |

Надпись обычно входит в сам логотип, поэтому текста рядом с ним нет.
Если логотип удалить, вернётся знак «EnRentAuto»; если удалить фон —
прежний тёмный градиент.

Поверх фотографии ложится затемнение (`.hero-scrim-x` и `.hero-scrim-y`
в `globals.css`): плотное со стороны текста, прозрачное с противоположной,
поэтому заголовок и форма поиска читаются на любом снимке. В арабской версии
стороны меняются местами, на телефоне затемнение становится сплошным.
**Главный объект снимка лучше держать справа** — слева его закроет текст.
...
- каждой картинки хранится ровно одна: при загрузке прежняя удаляется,
...
scp ~/Downloads/hero.jpg root@178.209.127.144:/tmp/hero.jpg

# на сервере
mkdir -p /opt/enrentauto/data/uploads/brand
mv /tmp/logo.png /opt/enrentauto/data/uploads/brand/logo.png
mv /tmp/hero.jpg /opt/enrentauto/data/uploads/brand/hero.jpg
