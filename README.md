...
│           ├── admin/hero-fit/route.ts       PUT — как показывать фон
...
Переключатель в том же разделе админки:
...
Настройка хранится в `Settings.heroFit` (единственная строка «singleton»)
и действует и для фотографии, и для видео.
