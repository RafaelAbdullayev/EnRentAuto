...
Достаточно одного ключа в `.env`:

```bash
# Claude — переводит живее, особенно на азербайджанский
ANTHROPIC_API_KEY="sk-ant-..."
# TRANSLATE_MODEL="claude-opus-5"   # необязательно

# либо Google Cloud Translation v2
GOOGLE_TRANSLATE_API_KEY="..."
```

Ключа нет — кнопки перевода в админке не показываются, а вместо них написано,
что перевод не настроен. Всё остальное работает: переводы вписываются руками,
непереведённые языки показывают русский текст.

Расход небольшой: описание одного автомобиля — несколько сотен знаков,
